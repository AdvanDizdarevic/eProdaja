...
namespace eProdaja_API.Util
{
    public class RecomendSystem
    {
        ...
        public List<esp_Proizvodi_SelectById_Result> GetSlicneProizvode(int proizvodId)
        {
            ...
            //Lista preporučenih proizvoda uzima top 5 proizvoda sa najboljoj ocjenom
            return preporuceno.OrderByDescending(x => x.ProsjecnaOcjena).Take(5).ToList();
        }
        ...
        {
            List<Proizvodi> aktivniProizvodi = db.Proizvodis.Where(x => x.Status == true && x.ProizvodID != proizvodId).ToList();
